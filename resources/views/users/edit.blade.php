...
@section('heading', 'Sửa User')
@section('content')
    <form class="space-y-6 max-w-2xl" method="POST" action="{{ route('users.update', $user->id) }}">
        @csrf
        @method('PUT')
        <div>
        ...
        <div>
            <label class="block text-sm font-medium mb-1">Mật khẩu mới (bỏ qua nếu không đổi)</label>
        ...
        <div class="flex items-center gap-3">
            <button class="rounded-xl bg-brand-600 text-white px-5 py-2.5 font-semibold hover:bg-brand-700">Cập nhật</button>
