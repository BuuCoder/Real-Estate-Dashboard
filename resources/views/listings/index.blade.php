                                ...
                                <td class="px-6 py-4 ">
                                    <div class="flex items-center gap-2">
                                        <a href="{{ route('listings.show', $listing) }}"
                                            class="rounded-xl bg-blue-500 text-white px-3 py-2 font-semibold hover:bg-blue-600 transition text-xs md:text-md flex items-center justify-center" title="Chi tiết">
                                        ...
                                        <a href="{{ route('listings.edit', $listing) }}"
                                            class="rounded-xl bg-yellow-400 text-white px-3 py-2 font-semibold hover:bg-yellow-500 transition text-xs md:text-md flex items-center justify-center" title="Sửa">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                            </svg>
                                        </a>
                                        <button type="button"
                                            onclick="openPopup(); window.deleteForm = document.getElementById('deleteFormListing{{ $listing->id }}');"
                                            class="rounded-xl bg-red-500 text-white px-3 py-2 font-semibold hover:bg-red-600 transition text-xs md:text-md flex items-center justify-center" title="Xóa">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                                            </svg>
                                        </button>
                                        <form id="deleteFormListing{{ $listing->id }}" class="hidden" action="{{ route('listings.destroy', $listing) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </div>
                ...
                @foreach ($listings as $listing)
                    <div class="bg-white rounded-3xl shadow-panel p-4 flex flex-col">
                        ...
                        <div class="flex gap-2 mt-4">
                            ...
                            <button type="button"
                                onclick="openPopup(); window.deleteForm = document.getElementById('deleteFormCard{{ $listing->id }}');"
                                class="rounded-xl bg-red-500 text-white px-3 py-2 font-semibold hover:bg-red-600 transition text-xs md:text-md flex items-center justify-center" title="Xóa">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                                </svg>
                            </button>
                            <form id="deleteFormCard{{ $listing->id }}" class="hidden" action="{{ route('listings.destroy', $listing) }}" method="POST">
                                @csrf
                                @method('DELETE')
                            </form>
                        </div>
