                ...
                <tr class="hover:bg-gray-50 transition">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex gap-2 h-full items-center">
                            <a href="{{ route('posts.show', $post) }}" class="rounded-xl bg-blue-500 text-white px-3 py-2 font-semibold hover:bg-blue-600 transition text-xs md:text-md h-full flex items-center justify-center" title="Chi tiết">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                                </svg>
                            </a>
                            <a href="{{ route('posts.edit', $post) }}" class="rounded-xl bg-yellow-400 text-white px-3 py-2 font-semibold hover:bg-yellow-500 transition text-xs md:text-md h-full flex items-center justify-center" title="Sửa">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                </svg>
                            </a>
                            <button type="button"
                                onclick="openPopup(); window.deleteForm = document.getElementById('deleteForm{{ $post->id }}');"
                                class="rounded-xl bg-red-500 text-white px-3 py-2 font-semibold hover:bg-red-600 transition text-xs md:text-md h-full flex items-center justify-center" title="Xóa">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                                </svg>
                            </button>
                            <form id="deleteForm{{ $post->id }}" class="hidden" action="{{ route('posts.destroy', $post) }}" method="POST">
                                @csrf
                                @method('DELETE')
                            </form>
                        </div>
